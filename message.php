...
if ($qRes1-> rowCount() == 0) {
  ...
  print "<textarea style='width: 64%; padding: 5px; margin: auto;' name='msgInput' class='form-control' placeholder='Message text' required></textarea>";
  print "<button class='btn btn-lg btn-primary btn-block' style='padding: 5px; width: 64%; margin: auto;'type='submit'>Send Message</button>";
  print "</form>";
  
  print "<form class='form-signin' method='post' action='http://www.cs.gettysburg.edu/~mirari01/cs360project/cs360-project/message.php/?friend=$friend'>";
  print "<button class='btn btn-lg btn-secondary btn-block' style='width: 64%; padding: 5px; margin: auto;'type='submit'>Refresh Chat</button>";
  print "</form>";
  ...
  function printChat($uid, $friend, $db) {
    ...
    while ($row = $qRes->fetch()) {
      ...
      if ($sender==$uid) {
        ...
        print "<ul class='list-group' style='color: #5bc0de; width: 64%; margin: auto;'><li class='list-group-item'><h3 style='text-align: left; margin: auto'>$message</h3></li></ul>";
        print "<br>";
      }
    }
